  ...
  try
    ...
    if (size (pkgfcns, 2) == 3)
      ...
      if (! isempty (url))
        ...
        idx = strfind (cls_text, "</div>")(end);
        cls_text = [cls_text(1:idx-1), url_text, cls_text(idx:end)];
      endif
    endif
  catch
...
%!        '    ## First method.', ...
%!        '    ## @seealso{m2, BistFlat, BistFlat.m1, BistDerived.OwnProp,', ...
...
%! fprintf (fid, "%s\n", src{:});
%! fclose (fid);
%! ## A class declaring its methods at the first column, which is where a
%! ## declaration was invisible while they were searched for with a leading
%! ## space.
%! src = {'## -*- texinfo -*-', ...
%!        '## @deftypefn {} {@var{obj} =} BistFlush ()', ...
%!        '## A fixture class declaring at the first column.', ...
%!        '## @end deftypefn', ...
%!        'classdef BistFlush', ...
%!        '  methods', ...
%!        '## -*- texinfo -*-', ...
...
%!        '## Construct it.', ...
%!        '## @end deftypefn', ...
%!        'function this = BistFlush ()', ...
%!        'endfunction', ...
%!        '', ...
%!        '## -*- texinfo -*-', ...
%!        '## @deftypefn {} {} fm1 (@var{obj})', ...
%!        '## The first method.', ...
%!        '## @end deftypefn', ...
%!        'function fm1 (this)', ...
%!        'endfunction', ...
...
%!        '  endmethods', ...
%!        'endclassdef'};
%! fid = fopen (fullfile (d, "BistFlush.m"), "w");
%! fprintf (fid, "%s\n", src{:});
%! fclose (fid);
...
%! ## A package build gives every entry its source URL in a third column.
%! url = "https://github.com/bist/bist/blob/0123abc/inst/";
%! pf(:,3) = cellfun (@(n) [url, n, ".m"], pf(:,1), "UniformOutput", false);
%! oldpwd = pwd ();
%! unwind_protect
%!   cd (d);
...
%!test  # a documented name links to its page, never to its source URL
%! assert (! isempty (strfind (mthd, "href=\"BistFlat.html\">BistFlat</a>")));
%! assert (isempty (strfind (mthd, "https:__github.com")));
...
%! d = fullfile (tempdir (), "pkg_octave_doc_cls_bist");
%! src = {'## -*- texinfo -*-', ...
%!        '## @deftypefn {} {@var{obj} =} BistTabled ()', ...
%!        '## A fixture class whose help holds a table.', ...
...
%!        '  methods (Access = public)', ...
%!        '    ## -*- texinfo -*-', ...
%!        '    ## @deftypefn {} {} t1 (@var{obj})', ...
%!        '    ## A method whose help holds a table.', ...
%!        '    ## @multitable @columnfractions 0.5 0.5', ...
%!        '    ## @item a @tab b', ...
%!        '    ## @end multitable', ...
%!        '    ## @seealso{BistFlat}', ...
...
%! unwind_protect
%!   cd (d);
%!   classdef_texi2html ("BistTabled", pf3, info);
%!   cls = fileread ("BistTabled.html");
%!   mth = fileread ("BistTabled.t1.html");
%! unwind_protect_cleanup
%!   cd (oldpwd);
%! end_unwind_protect
%! for page = {cls, mth}
%!   at = strfind (page{1}, "Source Code:");
%!   assert (numel (at), 1);
%!   assert (at > strfind (page{1}, "See also:")(1));
%!   assert (numel (strfind (page{1}, "<div")), ...
%!           numel (strfind (page{1}, "</div>")));
%! endfor
...
%! unwind_protect
%!   cd (d);
%!   classdef_texi2html ("BistTabled", {"BistTabled", "Cat"}, info);
%!   cls = fileread ("BistTabled.html");
%!   mth = fileread ("BistTabled.t1.html");
%! unwind_protect_cleanup
%!   cd (oldpwd);
%! end_unwind_protect
%! for page = {cls, mth}
%!   assert (isempty (strfind (page{1}, "Source Code:")));
%!   assert (numel (strfind (page{1}, "<div")), ...
%!           numel (strfind (page{1}, "</div>")));
%! endfor
