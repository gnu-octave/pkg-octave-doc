...
includes collapsible documentation for all class properties returned by calling
`properties ('classname')` as well as collapsible documentation only for the methods
...
**A `doc-cache` entry**, which is what `lookfor` searches. A cache holds the class, its constructor where the file declares one, the methods the class file declares and the properties `properties` reports, inherited properties included. An inherited method is not cached, `help` refusing its docstring where it answers for an inherited property. The constructor is the only hidden member that is ever cached. `INDEX` gates this at folder and package scope, so a name it does not list is skipped and reported.
...
| A public method or property the class declares | required | yes | yes |
| A public property the class inherits | on the declaring class | yes | yes |
| A public method the class inherits | on the declaring class | **no** | **no** |
...
The two inherited rows follow what `help` answers: it gives an inherited property's docstring and refuses an inherited method's, so a subclass is documented with its inherited properties and with none of its inherited methods, and a cross-reference naming one of those methods renders as plain text rather than as a link.
...
* `@math{}` texi tags are converted to `<math></math>` tags in HTML and their contents are scanned for the `*` character, which is replaced by `&times;` in order to properly display the multiplication symbol. Use `*` to denote multiplication inside `@math{}` tags; the letter `x` is left untouched so it can be used freely as a variable name (for example `@math{exp (-2 * x)}` renders as `exp (-2 × x)`). This scanning only affects the contents of `@math{}` texi tags. (Note: release 0.4.7 also converted a lower-case `x` to `&times;`, but that clobbered variable names and identifiers such as `exp`, `max`, and `index`; the `x` conversion has since been removed and only `*` is converted.)
...
````
  Write the reference in this **short form** — it stays clean and self-evident in the command-line `help` output (where it plainly means "example 2 of *this* function/method"). When the page is generated, the converter automatically **expands** the bare `#example<N>` fragment to the fully-qualified anchor of the current owner — `#function_name-example2` on a function page, `#Class_method-example2` inside a method's docstring, and so on. (Every non-alphanumeric character of the owner name, such as the `.` in `Class.method`, is mapped to `_` in the anchor so the id stays valid in a CSS selector — a dotted id would break the collapse toggle.) Because every anchor is thus qualified by its owner, the fragments never collide even on a *lumped* classdef page where the constructor and several members' demos share one page. (This automatic expansion applies to `#example<N>` fragments only; to link to an example on a *different* page, write the full target yourself, e.g. `@url{function_name.html#function_name-example1, ...}`.)
